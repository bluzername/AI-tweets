...
class TelegramPublisher:
    ...
    def format_thread_as_message(self, tweets: List[str], podcast_name: str,
        ...
        # Join tweets with separators
        separator = "\n\n─────────────────\n\n"
        body = separator.join(tweets)
        ...
        if len(message) > 4096:
            # Truncate body, keep header
            max_body = 4096 - len(header) - 50  # Leave room for truncation note
            body = body[:max_body]
            # Cut at last separator to avoid mid-sentence cut
            last_sep = body.rfind('─────')
            if last_sep > 0:
                body = body[:last_sep]
            message = f"{header}\n{body}\n\n_(continued...)_"
    ...
    def publish_thread(self, tweets: List[str], podcast_name: str,
        ...
        # Format the message
        message = self.format_thread_as_message(tweets, podcast_name, episode_title)
        ...
        if thumbnail_path and Path(thumbnail_path).exists():
            # For photo posts, caption is limited to 1024 chars
            if len(message) <= 1024:
            ...
            # If caption too long, send photo first, then message
            photo_response = self.send_photo(thumbnail_path)
